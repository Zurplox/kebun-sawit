...
def build_viewer(paths, metas, stamp, ver, out_dir, day, health=None, timelapse=None):
    ...
    # CATATAN: dashboard app utama (dengan slider kecerahan/kontras + layout mobile/PC)
    # ada di index.html dan TIDAK boleh ditimpa. Dashboard lama ditulis ke dashboard.html.
...
def main():
    ...
    # Pilih scene "bebas awan" dari DAFTAR YANG SAMA dengan "terbaru" (anti-mismatch).
    # Aturan: tanggal TERBARU yang awannya <= ambang; jika tak ada, tanggal paling sedikit awan.
    clear_scenes = [(d, cc) for d, cc in scenes if cc is not None and cc <= CLOUDFREE_MAX_CC]
    if clear_scenes:
        cf_iso, cf_cc = clear_scenes[-1]
    else:
        known = [(d, cc) for d, cc in scenes if cc is not None]
        cf_iso, cf_cc = (min(known, key=lambda x: x[1]) if known else (mr_iso, mr_cc))
    # Invarian penting: "bebas awan" TIDAK BOLEH lebih baru dari "terbaru".
    # Jika sampai terjadi, itu tanda data tak konsisten -> paksa ikut tanggal terbaru.
    if cf_iso > mr_iso:
        cf_iso, cf_cc = mr_iso, mr_cc
    cf_label = nice_date(cf_iso)
    print("Bebas awan: %s (awan %s) | Terbaru: %s (awan %s)" %
          (cf_iso, "?" if cf_cc is None else str(cf_cc) + "%",
           mr_iso, "?" if mr_cc is None else str(mr_cc) + "%"))
    paths = []
    metas_latest = {}
